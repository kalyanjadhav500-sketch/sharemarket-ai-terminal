...
def fetch_multi_timeframe(symbol: str):
    return {
        "5m": fetch_history(symbol, "5d", "5m"),
        "15m": fetch_history(symbol, "1mo", "15m"),
        "1h": fetch_history(symbol, "3mo", "1h"),
        "1d": fetch_history(symbol, "2y", "1d"),
    }

def fetch_many(symbols, period="1mo", interval="15m", workers=6):
    out = {}
    with ThreadPoolExecutor(max_workers=workers) as ex:
        jobs = {ex.submit(fetch_history, s, period, interval): s for s in symbols}
